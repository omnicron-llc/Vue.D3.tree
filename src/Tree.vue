...
export default {
	...
	methods: {
		...
		transformSvg (g, size) {
			...
			return this.layout.transformSvg(g, this.margin, size, this.maxTextLength)
		},

		updateTransform (transform, size) {
			size = size || this.getSize()
			return this.layout.updateTransform(transform, this.margin, size, this.maxTextLength)
		},
		...
		updateGraph (source, {transitionDuration = undefined, resetPopUp = true} = {}) {
			...
			newNodes.attr('transform', d => `${translate(originBuilder(d), layout, transform)} scale(0.1)`)
				.append('g')
				.attr('class', 'node')
			...
			const allNodesPromise = toPromise(allNodes.transition().duration(transitionDuration)
				.attr('transform', d => `${translate(d, layout, transform)}`)
				.attr('opacity', 1))
			...
			const exitingNodesPromise = toPromise(exitingNodes.transition().duration(transitionDuration)
									.attr('transform', d => `${translate(forExit(d), layout, transform)}`)
			...
			const size = this.getSize()
			this.applyZoom(size)
			console.log(this.layout.layout.size())
			//this.layout.size(this.internaldata.tree, size, this.margin, this.maxTextLength)
	...
	computed: {
		tree () {
			...
			this.layout.size(tree, size, this.margin, this.maxTextLength)
			return tree
		},
